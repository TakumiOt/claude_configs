...
- The first `###` group is the shipped PBI itself: its heading is the PBI's slice sentence, and its bullets quote the PBI's slice-level 受け入れ基準 (the 動作確認 criteria) from the `## PBI:` section of `docs/tasks/<work-name>.md`.
...
- Each bullet is one measurable, verifiable criterion sourced verbatim from the PBI section or the bundled task's 受け入れ基準 cell in the タスク分解 table (`docs/tasks/<work-name>.md`). Do not invent criteria, do not silently absorb implementation drift.
