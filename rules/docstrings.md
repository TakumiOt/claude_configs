...
Prose is PROHIBITED for: enumerating 2+ parameters / errors / cases, walking through reasons or consequences in paragraph form, or narrating implementation mechanics. This mirrors the "Bullets First" rule `pr-style.md` and `design-doc-style.md` already enforce.
...
| Why this design beat the alternatives; historical migration | trade-off analysis, "what we tried before" | ADR / `design-notes.md` / `git blame` — NOT a docstring |

- **Item Why states purpose, not history** — the longer "why this way and not that way" goes to an ADR or design note, reachable via `git blame`, never pasted into the docstring.
...
Design documents (`docs/design/`) are **basic design** — they state each port's role, each error's domain meaning, and the layer contracts, but they do **not** contain docstring drafts. The `developer` agent writes every docstring **from scratch** in the code, in English, using the design document as context for the *Why* and the contract — not as text to transcribe.
...
- **External-artifact references are prohibited** as load-bearing context — do **not** cite ADR numbers, design-document paths, phase names, ticket IDs, or commit hashes.
...
- A docstring that merely echoes the design document's basic-design wording without being grounded in the actual implementation.
...
| Docstring that echoes the design document's basic-design wording when the implementation has revealed additional constraints not yet reflected | 🟡 suggestion ("stale docstring — ground it in the implementation") |
