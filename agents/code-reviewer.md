...
- **Mandatory checks (every review)**:
  ...
  4a. **Scope adherence (per `~/.claude/CLAUDE.md`)**:
      - **Path C (full orchestration)**: You are invoked **per task** in Phase 2 of the Orchestration Loop. The boundary is the current task's entry (its スコープ / 受け入れ基準) under the design doc's `## タスク分解` section, cited by ID in your invocation prompt. Read both the design doc and that task entry. Do NOT review against any PR document — PR documents are produced later in Phase 3 by `pr-writer` and graded by `pr-reviewer`, neither of which concerns code review. If the task ID is not in the prompt, ask the orchestrator before reviewing.
