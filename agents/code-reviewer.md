...
- **Mandatory checks (every review)**:
  ...
  4a. **Scope adherence (per `~/.claude/CLAUDE.md`)**:
      ...
      - **Common: no enforced line count** — do not run `git diff | wc -l` and do not flag size by line count. Instead apply qualitative signals (Path C task-level: ≤ 1 concept, ≤ ~3 files; Path B: ≤ 1 concept, ≤ 3 files):
        ...
        - **Task clearly too coarse** (Path C: concepts ≫ 1 or files ≫ 3 indicating the architect under-decomposed, even if every change is in scope): 🟡 suggesting the architect split the task in the design doc and re-decompose for next time. Route to `architect`. Do NOT block the task on size alone if the work itself is correct and in scope — flag it as a learning signal, not a gate.
  ...
  6a. **Test placement (Rust workspace)**: Per `~/.claude/rules/rust.md` "Test Layout", all integration and E2E tests live in the `test-integration` crate. Integration tests placed in a domain crate's `tests/` directory, or any `tests/` directory created in a non-`test-integration` workspace crate, → 🔴. Unit tests in `#[cfg(test)] mod tests { ... }` blocks within their owning crate are the expected pattern and not flagged.
