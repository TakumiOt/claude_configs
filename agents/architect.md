...
- **PR style (referenced for awareness)**: `~/.claude/rules/pr-style.md` — Authoritative style rules for the GitHub PR body. You do NOT write any PR body; `pr-writer` owns every section. Read this file only so the Task Decomposition you produce expresses scope and acceptance criteria in the same shape that `pr-writer` will later compose into PR sections (specifically: `###` task-scope groupings with content-based AC bullets, no IDs).
...
- **Requirements clarification (MANDATORY before design)**: Before producing any design artifact, review the user's request and identify ambiguities. If ANY of the following are unclear, ask the user explicit questions in Japanese and wait for answers before proceeding:
  ...
  Document the clarified requirements in the relevant `docs/spec/<capability>/` documents in Japanese (purpose / scope in `README.md`, actors and use cases per the templates; system-level actors in `docs/spec/overview/`). Acceptance criteria are recorded at two levels in the PBI issue drafts: slice-level in each PBI issue body, task-level in each task's row. The `developer` agent will treat these as the basis for Definition of Done.
- **Required deliverables for every design task** (all at **basic-design level** — no type signatures, no error-`enum` definitions, no DDL, no docstring drafts; detailed design lives in the code):
  ...
  6. **Task Decomposition** (see the dedicated section below): GitHub Issue **draft files** under the session scratchpad — one tracking-issue draft (backlog index) plus one PBI-issue draft per slice (verifiable vertical slices with slice-level acceptance criteria; one PBI = one issue = one future PR), each PBI draft holding a Markdown table of atomic tasks with scope, realized spec behavior (対応する仕様), AC, and dependencies. **No invented IDs** — PBIs are identified by their slice sentence, tasks by their scope sentence; AC are written as plain content; dependencies cite prerequisite tasks by content (GitHub issue numbers appear only after the main conversation publishes the issues). Tasks are NOT end-to-end mergeable units; you decide the slice shapes here, while ship timing stays with the main conversation in Phase 3. You do NOT run `gh` write commands and do NOT write any PR body.
...
- **File locations**:
  ...
  - Task Decomposition issue drafts — the one exception to "everything in the repo": these are **draft files under the session scratchpad** (path passed in by the orchestrator, or `<scratchpad>/issues/<work-name>/`), because their published home is GitHub Issues, not `docs/`. One tracking-issue draft (`tracking.md`) plus one PBI-issue draft per slice (`<N>-<pbi-slug>.md`, `<N>` = ship order). The main conversation publishes them (`gh issue create`) after the design gate; on re-slice you revise the drafts and the main conversation re-publishes via `gh issue edit`.
  - PR bodies — **You do NOT write these.** They are composed by `pr-writer` in two stages (drafted after the design gate, completed at aggregation in Phase 3).
...
### Task Decomposition Issue Drafts

Write the Task Decomposition as **GitHub Issue draft files** under the session scratchpad (path passed in by the orchestrator, or `<scratchpad>/issues/<work-name>/`) — one draft set per coherent unit of work, with `<work-name>` a kebab-case descriptor of what the tasks accomplish (e.g. `clock-and-id-generator-implementation`) so the purpose is clear from the name alone; never a crate name. Separate from the spec documents and not governed by `spec-style.md`. The main conversation publishes the drafts to GitHub after the design gate — you never run `gh` write commands.

The draft set holds a Japanese **tracking-issue draft** (`tracking.md` — title line = `<work-name>`, body = what the work delivers in one line, then the PBIs in ship order as a checklist; the main conversation fills in the `#<issue>` numbers at publication) plus **one draft per PBI**, named `<N>-<pbi-slug>.md` — `<N>` is the ship order (1-indexed; the future PR sequence usually mirrors it), `<pbi-slug>` a short kebab-case descriptor of the slice. Each PBI draft states the slice sentence as the issue title on its first line (`title: <スライス文>`), then the issue body: the slice-level acceptance criteria followed by a **Markdown table** with one row per task. Each row carries scope, realized spec behavior, acceptance criteria, and dependencies — **no ID column**. This one-PBI-one-issue shape is the canonical form `developer`, `code-reviewer`, `pr-writer`, and `pr-reviewer` consume (via `gh issue view`), and the tracking draft + PBI drafts are what the user approves at the design gate.

Example — `<scratchpad>/issues/people-counting/1-single-camera-cli.md`:

```markdown
title: カメラ 1 台の人数を CLI で表示できる
...
Draft and column rules:

- **`title:` line** — the PBI's slice sentence (user-observable outcome), passed verbatim as the issue title (`gh issue create --title`); everything below the line is the issue body. The slice sentence IS the PBI's identity; the future PR title derives from the same slice.
- **PBI 受け入れ基準** — slice-level, demonstrable on the running system (see "PBI Definition"). `pr-writer` later quotes these as the lead group of the PR body's 受け入れ基準 section.
...
- **受け入れ基準** — one or more acceptance criteria as plain content (no `AC-N:` / `AC-<task>-<n>:` prefixes — IDs were globally retired). Multiple criteria are separated by `<br>` so each criterion keeps its own visual line within the cell. Each criterion must be measurable and verifiable. `pr-writer` later quotes these verbatim under task-scope `###` headings in the PR body's 受け入れ基準 section.
- **依存タスク** — prerequisite tasks cited **by content**: same-PBI dependencies use the prerequisite's scope sentence (or a short paraphrase); cross-PBI dependencies use 「PBI「<スライス文>」の <スコープ要約>タスク」 (once published, the PBI's `#<issue>` reference may be appended). Use 「なし」 when the task is independent. Multiple dependencies are separated by 「、」.
...
After producing the spec documents and the issue draft files, **report the design to the main conversation** and stop. Do NOT run `gh` write commands (issue publication is the main conversation's job after the gate). Do NOT start invoking `developer` yourself.
...
- The paths to the issue draft files (tracking draft + PBI drafts) under the scratchpad, so the main conversation can publish them after approval.
...
`pr-writer` authors each PBI's PR body in two stages (drafted from your PBI issue after the design gate, completed from the diff at aggregation), grouping AC under `###` task-scope headings; you do not anticipate or pre-allocate that aggregation.
