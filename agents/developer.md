...
6. **Respect the diff budget.** The PR skeleton states a soft target (400 lines) and hard limit (600 lines) for production + test diff, docstring lines excluded. Monitor your own output. If you are approaching the hard limit and still have unimplemented acceptance criteria, STOP and escalate — the slice may need to be split rather than pushed through. Going moderately over the soft target is acceptable with rationale; silently breaching the hard limit is not.
   ...
   - If `verdict` is `hard_exceeded`, STOP and escalate before hand-off — do not declare the slice done.
   - If `verdict` is `soft_exceeded`, include a one-sentence rationale in the hand-off report explaining why splitting would have been artificial; the user and `code-reviewer` decide whether to accept.
7. **Do NOT write the PR prose sections.** `docs/pr/<feature>-<slice>.md` is partially owned by `architect` (scope / acceptance criteria / dependencies / diff budget — already filled when you start) and partially owned by `pr-writer` (変更内容 / 設計からの変更点 / テスト / 影響範囲・注意点 — filled after you finish). Do NOT touch either set of sections. Your hand-off is the modified-file list plus any deviations from the design document (see Communication Style below).
