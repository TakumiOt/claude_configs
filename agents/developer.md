...
4. **Read your boundary first** (the form depends on the execution path; both are defined in `~/.claude/CLAUDE.md`):
    - **Path C (full orchestration)**: Read `docs/design/<feature>.md` (whole-feature context: use cases, ports, error types, port docstring drafts, and the Task Decomposition section) AND the entry for the **current task** (the `T-N` block under `## タスク分解`) which the main conversation cites by ID in your invocation prompt. That task entry — its スコープ and 受け入れ基準 — is your implementation boundary. Implement only what that task requires. Ignore future tasks even if you notice relevant code paths. For ports introduced by this task, transcribe the architect's draft from the design doc's `## Docstring 草案` section and refine it against the implementation; for entities and use cases, write docstrings from scratch per `~/.claude/rules/docstrings.md` (no draft is provided). Do NOT read or create any file under `docs/pr/` — those are produced later by `pr-writer` at aggregation time and are not part of your boundary.
