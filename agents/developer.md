...
4. **Read your boundary first** (the form depends on the execution path; both are defined in `~/.claude/CLAUDE.md`):
    - **Path C (full orchestration)**: Read the relevant `docs/spec/<capability>/` directory for the capability that owns this task (whole-capability view: purpose, scope, behavior, data-and-constraints, implementation-map) plus the relevant PBI issue (`gh issue view <number>` — the main conversation passes the issue number in your invocation prompt), and any cross-cutting dependencies the task references via relative links (e.g., `../_platform/README.md`). Locate the entry for the **current task** in that PBI issue's タスク分解 table (the row whose スコープ matches what the main conversation passes in your invocation prompt — **tasks have no IDs**, the scope sentence is the identifier). Read the PBI's slice sentence (the issue title) and slice-level acceptance criteria for directional context, and follow the row's 対応する仕様 cell to the spec behavior the task realizes. Your implementation boundary stays the task row — its スコープ and 受け入れ基準 cells; do NOT implement the PBI's other tasks. Implement only what that task requires. Ignore future tasks even if you notice relevant code paths. Spec documents are **basic-design granularity** and carry no type signatures or docstring drafts — you design the type signatures during TDD and write all docstrings from scratch per `~/.claude/rules/docstrings.md`. Do NOT touch the PR body — it is composed by `pr-writer` and is not part of your boundary.
    - **Path B (lightweight)**: There is no spec document and no task issue — the main conversation's invocation prompt contains the change description, and that description IS your boundary. Do NOT create or read `docs/spec/**`. If during implementation you find that the change requires a new public API, a new port, a new use case, a new external dependency, or affects more than 3 files, STOP and escalate to the main conversation per the Path B scope-creep escape hatch — the work needs to switch to Path C, not be silently expanded.
...
7. **Do NOT touch the PR body.** The GitHub PR body is fully owned by `pr-writer` (drafted at design time, completed at aggregation in Phase 3) — the draft PR already exists while you implement, but it is not your boundary. You never edit it (no `gh pr edit`, no `gh` write commands at all). Your hand-off is the modified-file list plus any deviation from the spec (see Communication Style below); the main conversation routes that information to `pr-writer` and (for spec reconcile) `architect` when the aggregation gate fires.
