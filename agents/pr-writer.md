...
- `docs/spec/<capability>/` directories and `docs/tasks/<work-name>.md` files — Authoritative source for capability intent, use cases, ports, error policy, and the standalone Task Decomposition (`docs/tasks/<work-name>.md`, one file per unit of work, the filename describing the work). The shipped PBI's `## PBI:` section (its slice sentence, slice-level 受け入れ基準, and task table) is your primary boundary for the スコープ and 受け入れ基準 sections of the PR. Used to ground **背景・目的** and to cross-check whether the implementation deviated from the spec. A single PR may aggregate tasks across multiple capabilities — read every capability spec directory the bundled tasks touch.
- The **shipped PBI** passed in by the orchestrator — the PBI's slice sentence plus the scope sentences of its bundled tasks. **No IDs are used** — the PBI is identified by its slice sentence, tasks by their scope sentences. Locate the `## PBI:` section in the relevant `docs/tasks/<work-name>.md`, quote its slice-level 受け入れ基準 as the lead acceptance criteria, and quote each task row's 受け入れ基準 cell as the source of truth for what this PR ships.
