...
- Specification documents (`docs/spec/**`), ADRs (`docs/adr/**`), PR bodies, and Task Decomposition issues on GitHub: **Japanese**. Code snippets inside them stay in English.
...
1. **architect** — Design phase. Produces and maintains the **living specification (仕様書)** at basic-design granularity: the system-wide `docs/spec/overview/` (全体仕様) and per-capability `docs/spec/<capability>/` directories (cross-cutting technical concerns in `docs/spec/_platform/`), plus the Task Decomposition as **GitHub Issue draft files** under the session scratchpad (one tracking-issue draft per unit of work + one PBI-issue draft per slice; the main conversation publishes them to GitHub after the design gate). Spec documents stay at basic-design granularity — purpose, responsibility, behavior, structure — and never contain type signatures, error-enum definitions, DDL, or docstring drafts (detailed design lives in the code). The architect drafts the spec forward at design time (target state) and reconciles it against the shipped implementation at merge (Phase 3); rationale ("why") lives in ADRs, not the spec. A single feature may touch multiple capabilities and therefore update multiple directories. Shapes the PBI slicing (each PBI = one future PR); ship timing stays with the main conversation. Does NOT write PR documents, does NOT run `gh` write commands, does NOT write implementation code.
2. **developer** — Implementation phase. Implements **one task at a time** per invocation, scoped to that task's row in its PBI issue (the PBI's GitHub Issue body; read via `gh issue view <number>`). Reads `docs/spec/<capability>/` for the capability the task belongs to, plus any cross-cutting dependencies the task references (e.g. `docs/spec/_platform/`). Writes tests and code across all Clean Architecture layers using BDD + Detroit-school TDD (Red → Green → Refactor). Writes docstrings from scratch per `~/.claude/rules/docstrings.md` (spec documents carry no drafts). Does NOT touch the PR body.
3. **pr-writer** — PR authoring phase, invoked in **two stages** per PBI. The deliverable is the **PR body itself** (no file under `docs/` — the body is composed in a scratchpad file at the path the main conversation passes in, then published by the main conversation via `--body-file`). **Stage 1 (draft composition, Phase 1 step 3)**: composes the body with the design-grounded sections (背景・目的 / スコープ / 受け入れ基準 / 依存PR / 関連ドキュメント) filled from the PBI issue and the touched spec directories; the implementation-dependent sections (変更内容 / 仕様からの変更点 / テスト / 影響範囲・注意点) each carry only the canonical placeholder line 「実装完了後に記載。」. **Stage 2 (completion update, Phase 3)**: recomposes the same body — fills the implementation-dependent sections grounded in the cumulative diff, and reconciles the design-grounded sections against what actually shipped (mismatches route to `architect`, never silently rewritten). Both stages follow `~/.claude/rules/pr-style.md`.
4. **code-reviewer** — Code review phase. Invoked per task (Phase 2) — reviews the task's code changes for architecture compliance, dependency health, scope adherence (boundary = the task row in its PBI issue), and business application concerns. Does NOT review PR bodies and does NOT modify code.
5. **pr-reviewer** — PR body review phase. Invoked per aggregation (Phase 3) — independently reviews the stage-2 PR body (the scratchpad file, before publication) for style compliance against `~/.claude/rules/pr-style.md` AND factual consistency against the shipped PBI issue, the cumulative diff, and **every capability spec directory the bundled tasks touch**. Does NOT review code quality and does NOT modify the PR body.

### Lightweight Path (Path B)

For small, scoped changes the main conversation invokes ONLY `developer` and `code-reviewer` — `architect`, `pr-writer`, `pr-reviewer`, the spec document, the structured PR body, and task decomposition are all skipped.
...
- Each task carries: a one-sentence scope, the spec behavior it realizes (対応する仕様 — the use case / behavior name in `docs/spec/<capability>/`, or the port / concern for `_platform` work), one or more acceptance criteria, and explicit task-level dependencies. **No invented identifiers** — neither tasks, ACs, use cases, nor ports carry numeric IDs. References across the spec directory (and into the PR body) use the natural identifier of the thing being referenced: function name for use cases (`IngestCountingEvents`), trait name for ports (`CountingEventRepository`), scope sentence for tasks ("the task that defines the `Clock` port"), slice sentence for PBIs. Cross-capability dependencies cite the other directory by name (e.g., "`_platform` の `Clock` ポート定義タスク"). The only numeric reference is the GitHub-native issue number (`#<issue>`), used for cross-linking a PBI (tracking checklist, `Closes #<issue>`) — never as a task or AC identifier.
...
- `developer` implements exactly one task per invocation. The main conversation passes the relevant task row (the scope sentence and its AC entries) plus the owning PBI's slice sentence and issue number to the developer in the invocation prompt. Future tasks are ignored even if visible from the current code path.

**Documented as GitHub Issues** — the Task Decomposition lives on GitHub, not under `docs/` (it is 使い捨て per the 時計分離 rule; the repo keeps only the living spec and ADRs). Two issue kinds per coherent unit of work, `<work-name>` a kebab-case descriptor of what the tasks accomplish, NOT a crate name:

- **Tracking issue** (1 per unit of work) — the backlog index. Title = `<work-name>`; body (Japanese) = what the work delivers in one line, then the PBIs in recommended ship order as a checklist (`- [ ] #<issue> — <スライス文>`), cross-PBI dependencies noted per line. Replaces the former `docs/tasks/<work-name>/README.md`.
- **PBI issue** (1 per slice, one PBI = one PR) — title = the PBI's slice sentence; body (Japanese) = the slice-level acceptance criteria plus the task table (format defined in `~/.claude/agents/architect.md`). Labeled or linked from the tracking issue; closed automatically by the PR via `Closes #<issue>`.

`architect` drafts the issue bodies as files under the session scratchpad and reports them at the design gate; after approval the **main conversation** publishes them (`gh issue create --title ... --body-file ...`) and later edits them on re-slice (`gh issue edit <number> --body-file ...`). Subagents never run `gh` write commands. There is **no PR document file**: the PR body is composed by `pr-writer` in two stages — drafted right after the design gate (Phase 1 step 3) and completed at aggregation (Phase 3) — and published by the main conversation via `--body-file`.

**Documentation directory layout (MANDATORY)**: The repo keeps only the two durable clocks — the living specification under `docs/spec/`, indexed by **capability**, and ADRs flat at `docs/adr/` (the home of all "why" per the 時計分離 rule). A system-wide `overview/` directory holds the 全体仕様; each capability gets its own directory; cross-cutting technical concerns (infrastructure, shared kernel) live in `_platform/`. The disposable clocks live on GitHub: Task Decomposition as Issues (tracking issue + PBI issues, see above), the per-PR diff narrative as the PR body. `docs/tasks/` and `docs/pr/` no longer exist.
...
**README everywhere (MANDATORY)**: Every directory under `docs/` — `spec/` and its subdirectories, and `adr/` — carries a Japanese `README.md` entry point stating 目的・責務, 収録方針 (what documents belong there and what does NOT), and a 目次 (relative links to the directory's contents). Owner: `architect`. The `docs/adr/README.md` is the single chronological ADR index — the 全体仕様 links to it rather than keeping a separate index.
...
- Task Decomposition lives in GitHub Issues (tracking issue + PBI issues) with its own format (`~/.claude/agents/architect.md`), not part of the spec templates and not governed by `spec-style.md`.
- Cross-directory references use relative Markdown links (e.g. `../_platform/README.md`).

PR bodies are composed by `pr-writer` in two stages: drafted when a PBI's implementation starts (design-grounded sections + placeholders), completed at aggregation (implementation-grounded sections). The body always references its PBI issue with a `Closes #<issue>` line so the merge closes the PBI automatically.
...
1. **architect** → drafts the spec forward (target state) for **every capability the feature touches**:
   ...
   - Task Decomposition: drafts the GitHub Issue bodies under the session scratchpad — one tracking-issue draft (backlog index) plus one PBI-issue draft per slice (format defined in `~/.claude/agents/architect.md`). Publishing to GitHub is the main conversation's job after the design gate.
   - Cross-directory references point to the other directory via relative Markdown links instead of duplicating content.
   - Does NOT run `gh` write commands. Does NOT decide PR aggregation. Does NOT write implementation code.
...
2a. **Issue publication (after gate approval, MANDATORY)** — the main conversation publishes the approved Task Decomposition to GitHub: `gh issue create` for each PBI issue (title = slice sentence, body from the architect's draft file), then the tracking issue (body listing the PBI issues in ship order as `- [ ] #<issue> — <スライス文>`). Reports the issue numbers to the user. On re-slice, the main conversation realigns via `gh issue edit <number> --body-file ...` after the revised backlog passes the gate again.
3. **Draft PR opening (per PBI, MANDATORY)** — when a PBI's implementation is about to start (its `feature/*` branch created and published per "Git Operations", the design artifacts committed):
   - **pr-writer (stage 1)** → composes the PR body into the scratchpad file the main conversation passes in, from the PBI issue and the touched spec directories: design-grounded sections (背景・目的 / スコープ / 受け入れ基準 / 依存PR / 関連ドキュメント) filled, implementation-dependent sections (変更内容 / 仕様からの変更点 / テスト / 影響範囲・注意点) set to the canonical placeholder 「実装完了後に記載。」, and the `Closes #<PBI issue>` line at the end.
   - The main conversation opens the draft PR: `gh pr create --draft --body-file <scratchpad-body-file>`, title = the PBI's slice sentence condensed to 50 ± 5 characters.
   - If the PBI is later re-sliced, stage 1 re-runs to realign the body and the PR title (`gh pr edit <number> --body-file ...`) after the revised backlog passes the design gate.

#### Phase 2 — Per-Task Implementation Loop

For each task in dependency order (or in parallel when dependencies allow):

1. **developer** → implements the current task via TDD. Reads `docs/spec/<capability>/` for the capability that owns the task (whole-capability view) AND the task's row in its PBI issue (`gh issue view <number>`; the main conversation passes the issue number and the task row in the invocation prompt), plus any cross-cutting dependencies the task references via relative links. Implements only what that task's scope + AC require — future tasks are ignored. Writes docstrings from scratch per `~/.claude/rules/docstrings.md` (spec documents carry no drafts). Reports the modified file list AND any deviation from the spec with rationale. Does NOT touch the PR body.
2. **code-reviewer** → grades the code, tests, docstrings, dependencies, and scope adherence (boundary = the task row in its PBI issue). Findings route to `developer`. Returns findings categorized as 🔴 blocker / 🟡 suggestion / 💭 nit.
...
The per-task loop is lightweight: the PR body is not touched, neither `pr-writer` nor `pr-reviewer` is invoked. Phase 2 may execute many tasks in succession before Phase 3 fires, but every task transition passes through the commit & report step above.
...
2. **pr-writer (stage 2)** → recomposes the stage-1 PR body in the scratchpad file (the main conversation passes the file path, the PBI issue number, and the current PR number; the stage-1 shape is fetched via `gh pr view <number> --json body` when the file is absent). Reads **every capability spec directory the bundled tasks touch** (`docs/spec/<capability>/`), the shipped PBI's issue (its slice sentence, slice-level acceptance criteria, and task table), and the cumulative diff. Replaces every placeholder section (変更内容 / 仕様からの変更点 / テスト / 影響範囲・注意点) with diff-grounded content per `~/.claude/rules/pr-style.md`, and reconciles the design-grounded sections against what actually shipped — mismatches route to `architect` (spec / task drift) or `developer` (out-of-scope implementation), never silently absorbed. Reports the body-file path and any mismatch surfaced during self-check.
3. **pr-reviewer** → grades the stage-2 PR body (the scratchpad file) on two axes: style (against the `pr-style.md` Severity Matrix) and factual consistency (against the shipped PBI issue — its slice-level acceptance criteria and bundled task rows —, the cumulative diff, and **every capability spec directory the bundled tasks touch**, including that the spec was reconciled to match the implementation). Findings route to `pr-writer` (prose), `architect` (spec drift / task drift in any capability spec directory or the PBI issue), or `developer` (when a PR-body inconsistency reflects the implementation being out of task scope).
...
5. **Aggregation hand-off (MANDATORY)** — the main conversation pushes the final commits and updates the draft PR body to the completed, reviewed body (`gh pr edit <number> --body-file <scratchpad-body-file>`), checks the PBI's line in the tracking issue via `gh issue edit` (or notes it for the user when the merge will auto-close the PBI issue), then reports the aggregation summary to the user and stops: the bundled tasks (scope sentences), the PBI issue number, the end-to-end behavior the slice makes exercisable, the branch name, the commit list (`git log --oneline`), a diffstat, and the draft PR URL. Reviewing the draft, marking it ready, and merging are the user's review workflow, done on their side. The next aggregation re-enters Phase 3 with whatever tasks Phase 2 has completed since.
...
1. **Spec artifacts exist and are reconciled**: For every capability the change touches, `docs/spec/<capability>/` is written/updated as living-spec documents following the `~/.claude/rules/spec-style.md` per-spec-kind templates (each directory carrying a Japanese `README.md` entry point), and the Task Decomposition is published as GitHub Issues (tracking issue current, one PBI issue per slice). The system-wide `docs/spec/overview/` is updated when system-level structure changes. Every touched `docs/` directory carries its `README.md` entry point per "README everywhere" above (`docs/adr/README.md` present and current). At the merge gate the spec is reconciled so it matches the shipped implementation. ADRs created when applicable (rationale lives there, not in the spec).
1a. **Design gate cleared**: The spec + PBI-grouped Task Decomposition (PBI slice sentences with slice-level AC, task scope sentences with 対応する仕様, spec-coverage map, dependencies, ship order — no IDs) has been reported to the user AND explicitly approved BEFORE Phase 2 started (Path C Phase 1 step 2), and the approved backlog published to GitHub Issues (Phase 1 step 2a).
1b. **PR body completed per aggregation**: For each aggregation triggered in Phase 3, the PR body is fully populated by `pr-writer` (stage 2 completed — no 「実装完了後に記載。」 placeholder remains) and published via `gh pr edit --body-file`, with the `Closes #<PBI issue>` line present. Style compliance against `~/.claude/rules/pr-style.md` and factual consistency against the shipped PBI issue, the cumulative diff, and the spec document are confirmed by `pr-reviewer`.
1c. **PR is a verifiable vertical slice**: Each aggregation ships exactly one PBI and forms a vertical slice that can be exercised on the running system end-to-end (a UI flow, a CLI command, or a use case/endpoint reachable through the composition root) — not a horizontal layer with no caller. The PR body's テスト section shows how the slice is verified (integration / E2E / manual when no automated path exists), and the PBI's slice-level 受け入れ基準 are demonstrable on the running system, not only in unit tests.
...
8. **Independent reviews passed**: BOTH `code-reviewer` (code / tests / docstrings / dependencies / scope adherence) AND `pr-reviewer` (PR body style + factual consistency) reviewed the change; all 🔴 blockers resolved; 🟡 suggestions addressed or explicitly deferred with rationale.
...
IMPORTANT: Git write access is split three ways. The **main conversation** owns branch creation, staging, commits, pushing `feature/*` branches, draft PR creation (`gh pr create --draft`), and GitHub Issue publication for the Task Decomposition (`gh issue create` / `gh issue edit`). The **user** owns PR review, marking the PR ready, merge, history rewriting, and issue deletion. **Subagents own nothing** — no agent (`architect` / `developer` / `pr-writer` / `code-reviewer` / `pr-reviewer`) ever runs a state-modifying git or `gh` command; each stops at reporting its modified file list (read-only `gh issue view` / `gh pr view` are allowed).
...
- The PR body always comes from the `pr-writer`-composed body file (under the session scratchpad) via `--body-file`: the stage-1 content at creation, the completed body at aggregation (`gh pr edit <number> --body-file ...`). Inline `--body` is prohibited (blocked by `git-guard.sh`). Never hand-write a PR body in the main conversation.
- Task Decomposition issues are published only after the design gate: `gh issue create --title <slice sentence> --body-file <architect draft>` per PBI, then the tracking issue. Issue bodies always come from `architect`-drafted files — never hand-written inline.
...
- **PR style** (every task that composes or reviews a PR body): `~/.claude/rules/pr-style.md` — Core Rule (Bullets First), Formatting Constraints, Per-Section Style, and the Severity Matrix used by `pr-reviewer`. Read by `pr-writer` (composition) and `pr-reviewer` (enforcement).
